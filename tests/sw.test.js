...
function createWorker(fetchImpl) {
  ...
  const worker = {
    ...
    request(clientId, { fileId = 'fileA', range = 'bytes=100-199', signal, method = 'GET' } = {}) {
      const request = new Request(`https://app.test/__drive_media/${fileId}?mime=video%2Fmp4&resourceKey=raw-key&session=7`, {
...
test('401 refresh retries once with identical Range/resource key and fresh authorization', async () => {
  const worker = createWorker((url, options, attempt) => attempt === 1
    ? errorResponse(401)
    : new Response('original bytes', { status: 206, headers: { 'Content-Range': 'bytes 100-199/1000' } }));
  ...
  assert.equal(messages[0].forceRefresh, true);
  assert.equal(messages[0].clientId, 'A');
});

test('tokens and CLEAR_TOKEN are scoped to the source client', async () => {
  const worker = createWorker(() => new Response('ok', { status: 206 }));
