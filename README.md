...
3. 재생기가 요청한 `Range` 바이트 구간을 서비스 워커가 가로챕니다.
4. 서비스 워커가 메모리의 액세스 토큰을 붙여 Drive `files.get?alt=media`에 전달합니다.
5. Drive가 보낸 원본 파일 바이트를 변환 없이 재생기에 스트리밍합니다.

따라서 Drive 미리보기용 저화질 트랜스코딩본은 사용하지 않습니다.
...
- 원본 화질과 재생 가능성은 별개입니다. Safari가 원본 코덱 또는 컨테이너를 지원하지 않으면 원본 바이트를 받아도 재생할 수 없습니다.
