...
4. 재생기가 요청한 `Range` 바이트 구간을 서비스 워커가 가로채 필요한 부분만 전달하고, 실제 응답이 부분 전송인지 검증합니다.
5. 서비스 워커는 요청한 창에 속한 메모리 토큰만 Drive `files.get?alt=media`에 붙입니다. 401이면 같은 창에서만 토큰을 갱신하고 동일한 범위 요청을 한 번 재시도합니다.
6. 구간 전송이 반복 실패하거나 Range 응답이 유효하지 않으면, 지원 브라우저에서는 원본 전체를 앱 전용 임시 디스크(OPFS)에 청크 단위로 저장해 같은 파일을 재생합니다. 디스크 쓰기를 지원하지 않는 Safari 등에서는 작은 파일만 기기별 메모리 한도 안에서 보조 경로를 사용합니다.
7. Drive 정책이 원본 다운로드를 제한하거나, 원본 전송·임시 디스크·제한된 메모리 경로가 모두 소진되거나, 정상 원본 응답에서 브라우저 코덱 비호환이 확인된 경우에만 앱 안의 Google 호환 재생기로 자동 전환합니다. 이 경로는 Google 변환본일 수 있으므로 원본 화질을 확인할 수 없으며, 외부 Drive 페이지는 자동으로 열지 않습니다.
...
Range 경로와 임시 디스크·메모리 보조 경로는 모두 Drive 원본 파일 바이트를 그대로 사용합니다. Google 호환 재생은 원본 화질을 확인할 수 없는 마지막 수단이며, 앱 밖 Drive 열기는 사용자가 직접 선택할 때만 실행됩니다.

## 재생 우선순위

1. **Drive 원본 파일 · Range 무변환 전송**: 유효한 `206` Range 응답으로 필요한 바이트만 점진적으로 전송
2. **Drive 원본 파일 · 연속 전송**: Drive가 `200` 전체 응답으로 원본을 반환해 Range를 제공하지 않을 때, 원본 연속 전송으로 재생
3. **Drive 원본 파일 · 임시 디스크**: 구간 경로가 반복 실패했을 때 OPFS 쓰기 지원·남은 저장 공간을 확인한 뒤 원본 파일을 임시 저장하고, 파일 전환 또는 플레이어 종료 시 삭제
4. **Drive 원본 파일 · 메모리**: 쓰기 가능한 OPFS가 없을 때 모바일 24MiB·데스크톱 96MiB까지 자동, 사용자 확인 후 각각 64MiB·256MiB까지 허용
5. **Google 호환 재생 · 원본 화질 미확인**: 원본 코덱 비호환·다운로드 제한·안전 저장 한도 초과 또는 모든 원본 계층 실패 후에만 사용
6. **Drive 직접 열기**: 앱 내부 호환 재생도 불가능할 때 사용자가 선택하는 최종 탈출 경로
