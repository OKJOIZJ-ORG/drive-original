...
5. 재생기가 요청한 `Range` 바이트 구간은 서비스 워커가 가로채 필요한 부분만 전달하고, 실제 응답이 부분 전송인지 검증합니다. 서비스 워커는 요청한 창에 속한 메모리 토큰만 Drive `files.get?alt=media`에 붙이며, 401이면 같은 창에서만 토큰을 갱신하고 동일한 범위 요청을 한 번 재시도합니다.
